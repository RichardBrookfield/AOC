...
DROP PROCEDURE IF EXISTS [#up_AddWires];
DROP PROCEDURE IF EXISTS [#up_FindCrossings];
GO

CREATE PROCEDURE [#up_AddWires]
	@WireId	int, @Coords varchar(MAX)
AS
BEGIN
	DECLARE @x0			int = 0
			,@y0		int = 0
	...
	DECLARE c CURSOR LOCAL FORWARD_ONLY READ_ONLY FOR
		SELECT [Value] FROM STRING_SPLIT(@Coords, ',');

	OPEN c;
	FETCH NEXT FROM c INTO @step;

	WHILE @@FETCH_STATUS = 0
	BEGIN
		SELECT	@direction	= LEFT(@step, 1)
				,@offset	= CAST(SUBSTRING(@step,2,100) AS int);

		SELECT	@x1 = CASE
						WHEN @direction = 'R' THEN @x0 + @offset
						WHEN @direction = 'L' THEN @x0 - @offset
						ELSE @x0
						END
				,@y1 = CASE
						WHEN @direction = 'U' THEN @y0 + @offset
						WHEN @direction = 'D' THEN @y0 - @offset
						ELSE @y0
						END
		
		-- Ensure all the wires are orient from -x to +x and -y to +y
		-- This means that e.g. BETWEEN [x0] AND [x1] is always valid.
		INSERT INTO #Wires ([WireId],[X0],[Y0],[X1],[Y1],[Steps],[Reversed])
		SELECT	@WireId, @x0, @y0, @x1, @y1, @steps + @offset, 0
		WHERE	@x0 < @x1 OR @y0 < @y1
		UNION ALL
		SELECT	@WireId, @x1, @y1, @x0, @y0, @steps + @offset, 1
		WHERE	@x0 > @x1 OR @y0 > @y1
