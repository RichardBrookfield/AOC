...
def get_mapped_position(position, direction, max_row):
    ...
    for m in mapping:
        if m[0] <= position[0] <= m[2] and \
           m[1] <= position[1] <= m[3] and direction == m[4]:
        ...
        if in_range(position[0] + drow, m[5], m[7] + drow) and \
           in_range(position[1] + dcol, m[6], m[8] + dcol) and \
           direction == opposite(m[9]):
            # Ensure the offset is positive, despite the range
            offset = abs(m[5] - position[0]) if m[6] == m[8] \
                else abs(m[6] - position[1])
        ...
        if turning:
            ...
            while item:
                ...
                if iscube:
                    ...
                    if 0 <= new[0] < max_row and \
                       0 <= new[1] < max_column and \
                       board[new[0]][new[1]] == 0:
                        off_board = True

                    if not 0 <= new[0] < max_row or \
                       not 0 <= new[1] < max_column:
                        off_board = True

                    if off_board:
                        new, next_direction = \
                            get_mapped_position(new, direction, max_row)
                else:
                    new[0] %= max_row
                    new[1] %= max_column
