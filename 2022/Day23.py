...
def main(day: int, input_type: str):
    ...
    for line in lines:
        ...
        for column in [column for column in range(len(line))
                       if line[column] == '#']:
    ...
    while True:
        ...
        for e in range(len(elves)):
            if new_positions.count(new_positions[e]) > 1:
                for previous in [
                        previous for previous in range(len(elves))
        ...
        if round >= 50 and round % 10 == 0:
            different = len(
                [e for e in range(len(elves))
                    if new_positions[e] != elves[e]]
            )
        ...
        if round == 10:
            ...
            bounded = (max_coords[0] - min_coords[0] + 1) * \
                      (max_coords[1] - min_coords[1] + 1) - len(elves)
