...
DROP PROCEDURE IF EXISTS [#up_DecodeLine];

DROP FUNCTION IF EXISTS [dbo].[fn_InCommon];
DROP FUNCTION IF EXISTS [dbo].[fn_IsSubset];
DROP FUNCTION IF EXISTS [dbo].[fn_SortString];
GO
...
CREATE OR ALTER FUNCTION [dbo].[fn_InCommon]
(
	@s1		varchar(10)
	,@s2	varchar(10)
)
...
AS
BEGIN
	DECLARE	@common	int	= 0
			,@i		int = 1
			,@ch	varchar(2);

	WHILE @i <= LEN(@s1)
	BEGIN
		SELECT	@ch = SUBSTRING(@s1, @i, 1);

		IF CHARINDEX(@ch, @s2) > 0
			SELECT	@common += 1;
...
CREATE OR ALTER FUNCTION [dbo].[fn_IsSubset]
(
	@set		varchar(10)
	,@subset	varchar(10)
)
...
BEGIN
	...
	WHILE @i <= LEN(@set)
	BEGIN
		SELECT	@ch = SUBSTRING(@set, @i, 1);

		SELECT	@subset = REPLACE(@subset, @ch, '');

		SELECT	@i += 1;
	END

	RETURN IIF(LEN(@subset) = 0, 1, 0);
END
GO

CREATE OR ALTER FUNCTION [dbo].[fn_SortString]
(
	@input		varchar(10)
)
	RETURNS varchar(10)
...
BEGIN
	...
	WHILE LEN(@input) > 0
	BEGIN
		SELECT	@ch		= LEFT(@input, 1)
				,@i		= 2;

		WHILE @i <= LEN(@input)
		BEGIN
			IF SUBSTRING(@input, @i, 1) < @ch
				SELECT	@ch	= SUBSTRING(@input, @i, 1);
			SELECT	@i += 1;
		END

		SELECT	@output += @ch
				,@input	= REPLACE(@input, @ch, '');
	END
...
CREATE OR ALTER PROCEDURE [#up_DecodeLine]
	@input		varchar(MAX)
	,@output	int			OUTPUT
AS
BEGIN
	...
	INSERT INTO @Digit ([segments], [value])
	SELECT	[dbo].[fn_SortString]([value]), -1
	FROM	STRING_SPLIT(LEFT(@input, @pos-2), ' ');

	-- These are distinct from the outset
	UPDATE @Digit SET [value] = 1 WHERE LEN([segments]) = 2;
	UPDATE @Digit SET [value] = 4 WHERE LEN([segments]) = 4;
	UPDATE @Digit SET [value] = 7 WHERE LEN([segments]) = 3;
	UPDATE @Digit SET [value] = 8 WHERE LEN([segments]) = 7;

	-- Length 6 items first.
	-- 9 is a superset of 4
	UPDATE	[D]
	SET		[value] = 9
	FROM	@Digit [D]
	JOIN	@Digit [O]	ON	[O].[value]	= 4
						AND	[dbo].[fn_IsSubset]([D].[segments], [O].[segments]) = 1
	WHERE	LEN([D].[segments])	= 6
	AND		[D].[value]			= -1;
	...
	SET		[value] = 6
	FROM	@Digit [D]
	JOIN	@Digit [O]	ON	[O].[value]	= 1
						AND	[dbo].[fn_InCommon]([D].[segments], [O].[segments]) = 1
	WHERE	LEN([D].[segments])	= 6
	AND		[D].[value]			= -1;
	...
	-- Then the length 5 items.
	-- 3 is a superset of 7
	UPDATE	[D]
	SET		[value] = 3
	FROM	@Digit [D]
	JOIN	@Digit [O]	ON	[O].[value]	= 7
						AND	[dbo].[fn_IsSubset]([D].[segments], [O].[segments]) = 1
	WHERE	LEN([D].[segments])	= 5
	AND		[D].[value]			= -1;
	...
	SET		[value] = 5
	FROM	@Digit [D]
	JOIN	@Digit [O]	ON	[O].[value]	= 6
						AND	[dbo].[fn_IsSubset]([O].[segments], [D].[segments]) = 1
	WHERE	LEN([D].[segments])	= 5
	AND		[D].[value]			= -1;
	...
	INSERT INTO @Value ([segments])
	SELECT	[dbo].[fn_SortString]([value])
	FROM	STRING_SPLIT(RIGHT(@input, LEN(@input)-@pos-1), ' ');

	SELECT	@output = 0;

	DECLARE	@i	int = 1;

	WHILE @i <= (SELECT MAX([id]) FROM @Value)
	BEGIN
		SELECT	@output *= 10;

		SELECT	@output += [D].[value]
		FROM	@Value [V]
		JOIN	@Digit [D]	ON	[D].[segments]	= [V].[segments]
		WHERE	[V].[id]	= @i;
...
SELECT	[Part 1]	= COUNT(*)
FROM	[#Input]
CROSS APPLY	(	SELECT	[Pos] = CHARINDEX('|', [value])
			) [Pipe]
CROSS APPLY	(	SELECT	[Reading] = [value]
				FROM	STRING_SPLIT((SELECT SUBSTRING([value], [Pipe].[Pos]+2, 100)), ' ')
			) [Digit]
WHERE	LEN([Digit].[Reading]) IN (2, 4, 3, 7);		-- Segments in 1, 4, 7, 8
...
BEGIN
	...
	EXEC [#up_DecodeLine] @input = @line, @output = @output OUTPUT;
