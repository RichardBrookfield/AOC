...
INSERT INTO [#Called]
SELECT	[value]
FROM	STRING_SPLIT((SELECT [value] FROM [#Input] WHERE [id] = 1), ',');
...
BEGIN
	...
	(
		SELECT	[id] = ROW_NUMBER() OVER (PARTITION BY NULL ORDER BY (SELECT NULL))
				,[value]
		FROM	STRING_SPLIT((	SELECT	LTRIM(REPLACE([value], '  ', ' '))
								FROM	[#Input]
								WHERE	[id] = @i), ' ')
	)
	INSERT INTO [#Card] ([card], [row], [column], [value])
	SELECT	@card, @row, [id], [value]
	FROM	[Row];
...
-- Reset for selected calls.
SELECT	@i = 1;

DECLARE	@finished	int = 0;

WHILE @i < (SELECT MAX([id]) FROM [#Called])
BEGIN
	UPDATE	[C]
	SET		[value] += 1000
	FROM	[#Card] [C]
	JOIN	[#Called] [L]	ON	[L].[id]		= @i
							AND	[L].[Number]	= [C].[value];

	SELECT TOP (1) @finished = [card]
	FROM	(	SELECT		[card]
				FROM		[#Card]
				WHERE		[value] >= 1000
				GROUP BY	[card], [row]
...
SELECT		[Part 1] = (SELECT [Number] FROM [#Called] WHERE [id] = @i)
							* SUM([value])
FROM		[#Card] [C]
WHERE		[card]		= @finished
AND			[value]		< 1000
...
BEGIN
	...
	IF (SELECT COUNT(*) FROM [#Unfinished]) = 1
		SELECT	TOP (1) @final = [card]
		FROM	[#Unfinished];

	IF (SELECT COUNT(*) FROM [#Unfinished]) = 0
		BREAK;

	SELECT	@i += 1;
END

SELECT		[Part 2] = (SELECT [Number] FROM [#Called] WHERE [id] = @i)
							* SUM([value])
FROM		[#Card] [C]
WHERE		[card]		= @final
AND			[value]		< 1000
