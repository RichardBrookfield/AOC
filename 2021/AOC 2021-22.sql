...
-- Awful brute force approach which takes about 50 minutes.

DROP TABLE IF EXISTS [#Input];
DROP TABLE IF EXISTS [#Action];
DROP TABLE IF EXISTS [#Core]
DROP TABLE IF EXISTS [#CoreExtra]
DROP TABLE IF EXISTS [#Segment];
DROP TABLE IF EXISTS [#XRange];
DROP TABLE IF EXISTS [#YRange];
DROP TABLE IF EXISTS [#ZRange];
...
(
	...
	,INDEX [idx_X] ([xLow], [xHigh], [yLow], [yHigh], [zLow], [zHigh], [on])
	,INDEX [idx_Y] ([yLow], [yHigh], [xLow], [xHigh], [zLow], [zHigh], [on])
	,INDEX [idx_Z] ([zLow], [zHigh], [xLow], [xHigh], [yLow], [yHigh], [on])
);
...
(
	...
	,CONSTRAINT [PK_Core] PRIMARY KEY ([x], [y], [z])
);

CREATE TABLE [#CoreExtra]
(
	[x]			int
	,[y]		int
	,[z]		int
	...
	,CONSTRAINT [PK_CoreExtra] PRIMARY KEY ([x], [y], [z])
);
...
(
	...
	,CONSTRAINT [PK_Segment] PRIMARY KEY ([xLow], [xHigh], [yLow], [yHigh], [zLow], [zHigh])
)
...
(
	...
	,CONSTRAINT [PK_XRange] PRIMARY KEY ([low], [high])
);
...
(
	...
	,CONSTRAINT [PK_YRange] PRIMARY KEY ([low], [high])
);
...
(
	...
	,CONSTRAINT [PK_ZRange] PRIMARY KEY ([low], [high])
);
...
SELECT	[value]
FROM	STRING_SPLIT(REPLACE(@InputT2, CHAR(13), ''), CHAR(10))
WHERE	[value] <> '';
...
BEGIN
	...
	SELECT	@on		= IIF([Space].[Pos] = 3, 1, 0)
			,@xLow	= CAST(SUBSTRING(@line, [X].[EqualPos] + 1, [X].[DotDotPos] - [X].[EqualPos] - 1) AS int)
			,@xHigh	= CAST(SUBSTRING(@line, [X].[DotDotPos] + 2, [X].[CommaPos] - [X].[DotDotPos] - 2) AS int)
			,@yLow	= CAST(SUBSTRING(@line, [Y].[EqualPos] + 1, [Y].[DotDotPos] - [Y].[EqualPos] - 1) AS int)
			,@yHigh	= CAST(SUBSTRING(@line, [Y].[DotDotPos] + 2, [Y].[CommaPos] - [Y].[DotDotPos] - 2) AS int)
			,@zLow	= CAST(SUBSTRING(@line, [Z].[EqualPos] + 1, [Z].[DotDotPos] - [Z].[EqualPos] - 1) AS int)
			,@zHigh	= CAST(RIGHT(@line, LEN(@line) - [Z].[DotDotPos] - 1) AS int)
	FROM	(SELECT [Line] = @line) [Input]
	CROSS APPLY	(	SELECT	[Pos]	= CHARINDEX(' ', @line)
				) [Space]
	CROSS APPLY	(	SELECT	[EqualPos]		= CHARINDEX('=',	@line, [Space].[Pos])
							,[DotDotPos]	= CHARINDEX('..',	@line, [Space].[Pos])
							,[CommaPos]		= CHARINDEX(',',	@line, [Space].[Pos])
				) [X]
	CROSS APPLY	(	SELECT	[EqualPos]		= CHARINDEX('=',	@line, [X].[CommaPos] + 1)
							,[DotDotPos]	= CHARINDEX('..',	@line, [X].[CommaPos] + 1)
							,[CommaPos]		= CHARINDEX(',',	@line, [X].[CommaPos] + 1)
				) [Y]
	CROSS APPLY	(	SELECT	[EqualPos]		= CHARINDEX('=',	@line, [Y].[CommaPos] + 1)
							,[DotDotPos]	= CHARINDEX('..',	@line, [Y].[CommaPos] + 1)
				) [Z]

	INSERT INTO	[#Action]
				([on], [xLow], [xHigh], [yLow], [yHigh], [zLow], [zHigh])
	VALUES		(@on, @xLow, @xHigh, @yLow, @yHigh, @zLow, @zHigh);

	SELECT	@i += 1;
END

-- Reset to process actions
SELECT	@i	= 1;
...
INSERT INTO	[#Core] ([x], [y], [z], [on])
SELECT	[X].[N], [Y].[N], [Z].[N], 0
FROM	[CoreNumbers] [X]
CROSS JOIN	[CoreNumbers] [Y]
CROSS JOIN	[CoreNumbers] [Z];

WHILE @i <= (SELECT MAX([id]) FROM [#Action])
BEGIN
	UPDATE	[C]
	SET		[on]	= [A].[on]
	FROM	[#Core] [C]
	INNER JOIN	[#Action] [A]	ON	[A].[id]	= @i
								AND	[C].[x]		BETWEEN [A].[xLow] AND [A].[xHigh]
								AND	[C].[y]		BETWEEN [A].[yLow] AND [A].[yHigh]
								AND	[C].[z]		BETWEEN [A].[zLow] AND [A].[zHigh];
...
INSERT INTO	[#XRange] ([low], [high])
SELECT		[Low], [High]
FROM		[XRange];

WITH [MinY] AS
(
	SELECT DISTINCT [Low] = [yLow]
	FROM	(	SELECT	[yLow]
				FROM	[#Action]
...
INSERT INTO	[#YRange] ([low], [high])
SELECT		[Low], [High]
FROM		[YRange];

WITH [MinZ] AS
(
	SELECT DISTINCT [Low] = [zLow]
	FROM	(	SELECT	[zLow]
				FROM	[#Action]
...
BEGIN
	...
	IF @on = 1
	BEGIN
		INSERT INTO	[#Segment] ([xLow], [xHigh], [yLow], [yHigh], [zLow], [zHigh])
		SELECT	[X].[low], [X].[high], [Y].[low], [Y].[high], [Z].[low], [Z].[high]
		FROM	[#XRange] [X]
		INNER JOIN	[#YRange] [Y]	ON	[Y].[low]	BETWEEN @yLow AND @yHigh
									AND	[Y].[high]	BETWEEN @yLow AND @yHigh
		INNER JOIN	[#ZRange] [Z]	ON	[Z].[low]	BETWEEN @zLow AND @zHigh
									AND	[Z].[high]	BETWEEN @zLow AND @zHigh
		WHERE	[X].[low]	BETWEEN @xLow AND @xHigh
		AND		[X].[high]	BETWEEN @xLow AND @xHigh
		AND		NOT EXISTS (
					SELECT	1
					FROM	[#Segment] [S]
					WHERE	[S].[xLow]	= [X].[low]
	...
	BEGIN
		DELETE	[S]
		FROM	[#Segment] [S]
		WHERE	[S].[xLow]	BETWEEN @xLow AND @xHigh
		AND		[S].[xHigh]	BETWEEN @xLow AND @xHigh
		AND		[S].[yLow]	BETWEEN @yLow AND @yHigh
		AND		[S].[yHigh]	BETWEEN @yLow AND @yHigh
		AND		[S].[zLow]	BETWEEN @zLow AND @zHigh
		AND		[S].[zHigh]	BETWEEN @zLow AND @zHigh;
	END

	SELECT @line = 'Completed action: ' + CAST(@i AS varchar(10));
	RAISERROR(@line,0,0) WITH NOWAIT;

	SELECT	@i += 1;
END

SELECT		[Part 2] = SUM([Calc].[cubes])
FROM		#Segment
CROSS APPLY (	SELECT	[cubes] = CAST([xHigh] - [xLow] + 1 AS bigint)
								* CAST([yHigh] - [yLow] + 1 AS bigint)
								* CAST([zHigh] - [zLow] + 1 AS bigint)
			) [Calc];

SELECT		[Part 2 Extra] = SUM([Calc].[cubes])
FROM		#Segment
CROSS APPLY (	SELECT	[cubes] = CAST([xHigh] - [xLow] + 1 AS bigint)
								* CAST([yHigh] - [yLow] + 1 AS bigint)
								* CAST([zHigh] - [zLow] + 1 AS bigint)
			) [Calc]
WHERE		[xLow] >= -50 AND [xHigh] <= 50
AND			[yLow] >= -50 AND [yHigh] <= 50
AND			[zLow] >= -50 AND [zHigh] <= 50;
