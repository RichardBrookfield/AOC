...
DECLARE [c] CURSOR LOCAL READ_ONLY FAST_FORWARD FORWARD_ONLY FOR
...
FETCH NEXT FROM [c] INTO @input;

WHILE @@FETCH_STATUS = 0
BEGIN
	SELECT	@posHyphen	= CHARINDEX('-', @input)
			,@posSpace	= CHARINDEX(' ', @input)
			,@posColon	= CHARINDEX(':', @input);

	IF @posHyphen = 0 OR @posSpace = 0 OR @posColon = 0
		THROW 50001, 'Error delimited not found', 0

	SELECT	@from		= TRY_CAST(SUBSTRING(@input, 1, @posHyphen-1) AS int)
			,@to		= TRY_CAST(SUBSTRING(@input, @posHyphen+1, @posSpace-@posHyphen) AS int)
			,@letter	= SUBSTRING(@input, @posSpace+1, 1)
			,@password	= SUBSTRING(@input, @posColon+2, 100)

	IF LEN(@password) - LEN(REPLACE(@password, @letter, '')) BETWEEN @from AND @to
		SELECT @valid1 += 1;

	IF	CASE WHEN SUBSTRING(@password, @from, 1) = @letter THEN 1 ELSE 0 END +
		CASE WHEN SUBSTRING(@password, @to,   1) = @letter THEN 1 ELSE 0 END = 1
	BEGIN
		SELECT @valid2 += 1;
	END
...
-- Parts 1 and 2: 591, 335
SELECT	[Valid Part 1] = @valid1
		,[Valid Part 2] = @valid2;
