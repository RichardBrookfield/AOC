...
BEGIN
	...
	BEGIN
		UPDATE	[L]
		SET		[L].[occupied] = [New].[Occupied]
		FROM	[#Layout] [L]
		LEFT JOIN	(	SELECT		[A].[From]
									,[Occupied] = SUM([L2].[occupied])
						FROM		[#Adjacent] [A]
						JOIN		[#Layout] [L2]	ON	[L2].[id]	= [A].[To]
						GROUP BY	[A].[From]
					) [Surround]	ON	[Surround].[From] = [L].[id]
		CROSS APPLY	(	SELECT	[Occupied]	= CASE	WHEN [L].[occupied] = 0 AND [Surround].[Occupied] IS NULL	THEN 1
													WHEN [L].[occupied] = 0 AND [Surround].[Occupied] = 0		THEN 1
													WHEN [L].[occupied] = 1 AND [Surround].[Occupied] > @limit	THEN 0
													ELSE [L].[occupied]
													END
					) [New]
		WHERE	[L].[occupied]	<> [New].[Occupied];
...
(
	...
	,INDEX [idx_Layout]		CLUSTERED		([row], [column], [occupied])
...
(
	...
	,INDEX [idx_Adjacent]	CLUSTERED		([from], [to])
)
...
DECLARE	@row		int = 1
		,@column	int
		,@maxrow	int	= (SELECT MAX([row]) FROM [#LayoutRaw])
		,@maxcolumn	int	= (SELECT MAX(LEN([layout])) FROM [#LayoutRaw]);
...
BEGIN
	...
	BEGIN
		IF (	SELECT	SUBSTRING([layout], @column, 1)
				FROM	[#LayoutRaw]
				WHERE	[row] = @row
				) = 'L'
		BEGIN
...
INSERT INTO [#Adjacent]
		([From], [To])
SELECT	[L].[id], [Adjacent].[id]
FROM	[#Layout] [L]
CROSS APPLY	(	SELECT	[A].[id]
				FROM	[#Layout] [A]
				WHERE	ABS([L].[row] - [A].[row]) <= 1
				AND		ABS([L].[column] - [A].[column]) <= 1
				AND		[A].[id] <> [L].[id]
			) [Adjacent];

EXEC [dbo].[#Process] @limit = 3;
...
-- Reset everything for the second part
TRUNCATE TABLE [#Adjacent];
UPDATE [#Layout] SET [occupied] = 0;
...
CROSS APPLY	(	SELECT TOP (1)
						[A].[id]
				FROM	[#Layout] [A]
				WHERE	[A].[row] - [L].[row] = [A].[column] - [L].[column]
				AND		[A].[row] > [L].[row]
				ORDER BY [A].[row]
			) [Adjacent];

-- Leading diagonal, up-left
INSERT INTO [#Adjacent]
		([From], [To])
SELECT	[L].[id], [Adjacent].[id]
FROM	[#Layout] [L]
...
CROSS APPLY	(	SELECT TOP (1)
						[A].[id]
				FROM	[#Layout] [A]
				WHERE	[A].[row] - [L].[row] = [L].[column] - [A].[column]
				AND		[A].[row] > [L].[row]
				ORDER BY [A].[row]
			) [Adjacent];

-- Trailing diagonal, down-left
INSERT INTO [#Adjacent]
		([From], [To])
SELECT	[L].[id], [Adjacent].[id]
FROM	[#Layout] [L]
