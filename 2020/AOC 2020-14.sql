...
DROP TABLE IF EXISTS [#memory2];
DROP PROCEDURE IF EXISTS [#up_BinaryToDecimal];
DROP PROCEDURE IF EXISTS [#up_DecimalToBinary];
DROP PROCEDURE IF EXISTS [#up_DecimalWithMask];
DROP PROCEDURE IF EXISTS [#up_WriteMemoryWithMask];
GO
...
CREATE PROCEDURE [#up_BinaryToDecimal]
	@binary			varchar(36)
	,@decimal		bigint			OUTPUT
AS
BEGIN
	SELECT	@decimal = 0;
	...
	BEGIN
		SELECT	@decimal	*= 2;
		SELECT	@decimal	+= CASE WHEN LEFT(@binary, 1) = '1' THEN 1 ELSE 0 END
		SELECT	@binary		= RIGHT(@binary, LEN(@binary)-1);
	END
END
GO

CREATE PROCEDURE [#up_DecimalToBinary]
	@decimal		bigint
	,@binary		varchar(36)	OUTPUT
AS
BEGIN
	SELECT	@binary = '';
	...
	WHILE @decimal > 0
	BEGIN
		SELECT	@binary		+= CASE WHEN @decimal % 2 = 0 THEN '0' ELSE '1' END;
		SELECT	@decimal	/= 2;
	END

	SELECT	@binary = RIGHT(REPLICATE('0', 36) + REVERSE(@binary), 36);
END
GO

CREATE PROCEDURE [#up_DecimalWithMask]
	@input			bigint
	,@mask			varchar(36)
	,@result		bigint		OUTPUT
AS
BEGIN
	DECLARE	@inputBinary	varchar(36)
			,@resultBinary	varchar(36)	= ''
			,@ch			char(1)
			,@i				int = 1;

	EXEC [dbo].[#up_DecimalToBinary] @decimal = @input, @binary = @inputBinary OUTPUT;

	WHILE @i <= LEN(@mask)
	BEGIN
		SELECT	@ch = SUBSTRING(@mask, @i, 1);
		
		IF @ch IN ('0', '1')
			SELECT	@resultBinary += @ch;
		ELSE
			SELECT	@resultBinary += SUBSTRING(@inputBinary, @i, 1);

		SELECT	@i += 1;
	END

	EXEC [dbo].[#up_BinaryToDecimal] @binary = @resultBinary, @decimal = @result OUTPUT;
END
GO

CREATE PROCEDURE [#up_WriteMemoryWithMask]
	@mask		varchar(36)
	,@offset	int
	,@value		int
AS
BEGIN
	DECLARE	@offsetBinary	varchar(36)	= ''
			,@offsetMasked	varchar(36) = ''
			,@i				int = 1
			,@ch			char(1)
			,@offsetDecimal	bigint
			;

	EXEC [dbo].[#up_DecimalToBinary] @decimal = @offset, @binary = @offsetBinary OUTPUT;

	WHILE @i <= LEN(@offsetBinary)
	BEGIN
		SELECT	@ch = SUBSTRING(@mask, @i, 1);

		IF @ch IN ('1', 'X')
			SELECT	@offsetMasked += @ch;
		ELSE
			SELECT	@offsetMasked += SUBSTRING(@offsetBinary, @i, 1);

		SELECT	@i += 1;
	END

	DECLARE	@offsetList table
    (
	...
	WITH [offsets] AS
	(
		SELECT	[Output]	= CAST('' AS varchar(36))
				,[Input]	= @offsetMasked
				,[Level]	= 0

		UNION ALL

		SELECT	CAST([Output] + [First].[Digit] AS varchar(36))
				,RIGHT([Input], 35-[Level])
				,[Level]+1
		FROM	[offsets] [O]
		CROSS APPLY	(	SELECT	[Digit] = SUBSTRING([Input],1,1)
					) [First]
		WHERE	[First].[Digit] IN ('0', '1')
		AND		[O].[Level]	< 36

		UNION ALL

		SELECT	CAST([Output] + '0' AS varchar(36))
				,RIGHT([Input], 35-[Level])
				,[Level]+1
		FROM	[offsets] [O]
		WHERE	SUBSTRING([Input],1,1) = 'x'
		AND		[O].[Level]	< 36
		UNION ALL

		SELECT	CAST([Output] + '1' AS varchar(36))
				,RIGHT([Input], 35-[Level])
				,[Level]+1
		FROM	[offsets] [O]
		WHERE	SUBSTRING([Input],1,1) = 'x'
	...
	WHILE @i <= (SELECT MAX([id]) FROM @offsetList)
	BEGIN
		SELECT	@offsetBinary = [value] 
		FROM	@offsetList
		WHERE	[id] = @i;

		EXEC [dbo].[#up_BinaryToDecimal] @binary = @offsetBinary, @decimal = @offsetDecimal OUTPUT;

		UPDATE	[#memory2]
		SET		[value]		= @value
		WHERE	[offset]	= @offsetDecimal;

		IF @@ROWCOUNT = 0
			INSERT INTO
					[#memory2] ([offset], [value])
			VALUES	(@offsetDecimal, @value);
...
BEGIN
	...
	IF LEFT(@line, 7) = 'mask = '
	BEGIN
		SELECT	@mask = RIGHT(@line, 36);
	END
	ELSE
	BEGIN
		SELECT	@posOpen	= CHARINDEX('[', @line)
				,@posClose	= CHARINDEX(']', @line);

		SELECT	@offset		= CAST(SUBSTRING(@line, @posOpen+1, @posClose-@posOpen-1) AS int)
				,@value		= CAST(SUBSTRING(@line, @posClose+4, LEN(@line)-@posClose-3) AS int);

		EXEC [dbo].[#up_DecimalWithMask]
					@input		= @value
					,@mask		= @mask
					,@result	= @valueMasked	OUTPUT;

		UPDATE	@memory1
		SET		[value]		= @valueMasked
		WHERE	[offset]	= @offset;

		IF @@ROWCOUNT = 0
			INSERT INTO
					@memory1 ([offset], [value])
			VALUES	(@offset, @valueMasked);

		-- Part 2 processing
		EXEC [dbo].[#up_WriteMemoryWithMask]
					@mask		= @mask
		            ,@offset	= @offset
		            ,@value		= @value;
	END
...
-- 13865835758282
SELECT	[Part 1] = SUM([value])
FROM	@memory1;

-- 4195339838136
SELECT	[Part 2] = SUM([value])
FROM	[#memory2];
