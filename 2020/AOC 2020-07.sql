...
DROP TABLE IF EXISTS [#bagRelationship];

DROP PROCEDURE IF EXISTS [#DescribeBag];
DROP PROCEDURE IF EXISTS [#DescribeBagAndNumber];
DROP PROCEDURE IF EXISTS [#GetBagID];
GO
...
CREATE TABLE [#bagRelationship]
(
	[containerID]	int				NOT NULL
	,[containedID]	int				NOT NULL
	,[number]		int				NOT NULL
);
GO

CREATE PROCEDURE [#DescribeBag]
	@input		varchar(200)
	,@colour	varchar(100)	OUTPUT
	,@modifier	varchar(100)	OUTPUT
	,@success	bit				OUTPUT
AS
BEGIN
	DECLARE	@pos1 int, @pos2 int;

	SELECT	@pos1 = CHARINDEX(' ', @input);
	SELECT	@pos2 = CHARINDEX(' ', @input, @pos1 + 1);
	...
	ELSE
	BEGIN
		SELECT	@modifier	= LEFT(@input, @pos1-1)
				,@colour	= SUBSTRING(@input, @pos1+1, @pos2-@pos1-1)
				,@success	= 1;
	END
END
GO

CREATE PROCEDURE [#DescribeBagAndNumber]
	@input		varchar(200)
	,@colour	varchar(100)	OUTPUT
	,@modifier	varchar(100)	OUTPUT
	,@number	int				OUTPUT
...
BEGIN
	...
	ELSE
	BEGIN
		SELECT	@number = TRY_CAST(LEFT(@input, @pos-1) AS int)
				,@input	= RIGHT(@input, LEN(@input) - @pos);
...
BEGIN
	IF LEN(@line) > 0
	BEGIN
		SELECT	@posContain = CHARINDEX(' contain ', @line);

		IF @posContain <= 0
			PRINT 'ERROR: could not find "contain"';
		ELSE
		BEGIN
			SELECT	@container	= LEFT(@line, @posContain-1)
					,@content	= REPLACE(
									RIGHT(@line, LEN(@line) - @posContain - 8)
									, '.', '');
			...
			IF @success = 0
			BEGIN
				PRINT 'ERROR: failed to identify containing bag'
			END
			ELSE
			BEGIN
				EXEC [dbo].[#GetBagID]
						@colour		= @colour
				        ,@modifier	= @modifier
				        ,@id		= @containerID	 OUTPUT;

				-- Special item to show no further content.
				IF @content = 'no other bags'
					SELECT	@content = '';
				
				WHILE LEN(@content) > 0
				BEGIN
					SELECT	@posComma = CHARINDEX(',', @content);

					SELECT	@subcontent = CASE WHEN @posComma <= 0
											THEN @content
											ELSE LEFT(@content, @posComma - 1)
											END;

					EXEC [dbo].[#DescribeBagAndNumber]
								@input		= @subcontent
								,@colour	= @colour	OUTPUT
								,@modifier	= @modifier	OUTPUT
					...
					IF @success = 0
					BEGIN
						PRINT 'ERROR: finding number of contained bags';
					END
					ELSE
					BEGIN
						EXEC [dbo].[#GetBagID]
								@colour		= @colour
								,@modifier	= @modifier
								,@id		= @containedID	 OUTPUT;

						INSERT INTO [#bagRelationship]
							([containerID], [containedID], [number])
						VALUES
							(@containerID, @containedID, @number);
					END

					SELECT	@content = CASE WHEN @posComma <= 0
										THEN ''
										ELSE RIGHT(@content, LEN(@content) - @posComma - 1)
										END;
				END
			END
		END
...
DECLARE	@shinygoldid int;
DECLARE	@containsGold table
(
	[id]	int		NOT NULL
);

SELECT	@shinygoldid = [id]
FROM	[#bagType]
WHERE	[modifier]	= 'shiny'
AND		[colour]	= 'gold'

DECLARE @localrowcount int = -1;

WHILE @localrowcount <> 0
BEGIN
	INSERT INTO @containsGold
	SELECT DISTINCT
			[containerID]
	FROM	[#bagRelationship]
	WHERE	(
			[containedID]	= @shinygoldid
	OR		[containedID]	IN (SELECT [id] FROM @containsGold)
			)
	AND		NOT EXISTS (SELECT 1 FROM @containsGold WHERE [id] = [containerID]);
...
DECLARE @metabags table
(
	[id]			int		NOT NULL
	,[number]		int		NOT NULL
	,[decomposed]	bit		NOT NULL
);

-- Same again...
DECLARE @newmetabags table
(
	[id]			int		NOT NULL
	,[number]		int		NOT NULL
	,[decomposed]	bit		NOT NULL
...
INSERT INTO @metabags ([id], [number], [decomposed])
	VALUES (@shinygoldid, 1, 0);

SELECT @localrowcount = -1;

WHILE @localrowcount <> 0
BEGIN
	DELETE FROM @newmetabags;

	INSERT INTO @newmetabags (
			[id]
			,[number]
			,[decomposed]
			)
	SELECT	[BR].[containedID]
			,[MB].[number] * [BR].[number]
			,0
	FROM	@metabags [MB]
	JOIN	[#bagRelationship] [BR]	ON	[MB].[id]	= [BR].[containerID]
	WHERE	[MB].[decomposed] = 0;
	...
	INSERT INTO @metabags
	SELECT	[id], [number], [decomposed]
	FROM	@newmetabags;

	SELECT	@localrowcount = @@ROWCOUNT;
END

-- We have to "remove" the original bag to find those contained.
SELECT	[Part 2] = SUM([number]) - 1
FROM	@metabags;
