...
DROP PROCEDURE IF EXISTS [#ValidateAttributes];

DROP FUNCTION IF EXISTS [AOC].[SplitAttribute];
...
CREATE FUNCTION [AOC].[SplitAttribute](
	@keyvaluepair	varchar(100)
)
RETURNS @pair table (
	[key]		varchar(20)
	,[value]	varchar(20)
...
AS
BEGIN
	DECLARE	@posColon	int = CHARINDEX(':', @keyvaluepair);

	IF @posColon > 1
		INSERT INTO @pair
		VALUES (SUBSTRING(@keyvaluepair, 1, @posColon-1), SUBSTRING(@keyvaluepair, @posColon+1, 100));

	RETURN;
END
GO

CREATE PROCEDURE [#ValidateAttributes](
	@fullvalidation	bit
	,@valid			bit		OUTPUT
)
AS
BEGIN
	...
	DECLARE @mandatoryAttributes table ( [key] varchar(20) );

	INSERT INTO @mandatoryAttributes ([key])
	VALUES	 ('byr')
			,('iyr')
			,('eyr')
			,('hgt')
	...
	SELECT @valid = CASE WHEN 7 = (
								SELECT	COUNT(*)
								FROM	[#Attributes] [A]
								JOIN	@mandatoryAttributes [MA]	ON	[MA].[key]	= [A].[key]
								)
					THEN 1 ELSE 0 END;

	IF @fullvalidation = 1 AND @valid = 1
	BEGIN
		DECLARE	@iValue int, @cValue varchar(20), @i int, @ch char(1);

		-- byr (Birth Year) - four digits; at least 1920 and at most 2002.
		SELECT	@iValue = ISNULL(TRY_CAST([value] AS int),0)
		FROM	[#Attributes]
		WHERE	[key] = 'byr';

		IF @iValue < 1920 OR @iValue > 2002
		BEGIN
			--PRINT 'Rejected birth: ' + CAST(@iValue AS varchar(20));
			SELECT @valid = 0;
		END
		...
		IF @iValue < 2010 OR @iValue > 2020
		BEGIN
			--PRINT 'Rejected issue: ' + CAST(@iValue AS varchar(20));
			SELECT @valid = 0;
		END
		...
		IF @iValue < 2020 OR @iValue > 2030
		BEGIN
			--PRINT 'Rejected expiry: ' + CAST(@iValue AS varchar(20));
			SELECT @valid = 0;
		END
		...
		SELECT @iValue = ISNULL(TRY_CAST(SUBSTRING(@cValue, 1, LEN(@cValue)-2) AS int), 0);

		IF RIGHT(@cValue, 2) NOT IN ('in', 'cm')
			OR RIGHT(@cValue, 2) = 'cm' AND (@iValue < 150 OR @iValue > 193)
			OR RIGHT(@cValue, 2) = 'in' AND (@iValue <  59 OR @iValue >  76)
		BEGIN
			--PRINT 'Rejected height: ' + ISNULL(@cValue, 'was null');
			SELECT @valid = 0;
		END
		...
		IF LEN(@cValue) <> 7
			OR SUBSTRING(@cValue, 1, 1) <> '#'
		BEGIN
			--PRINT 'Rejected hair colour:' + ISNULL(@cValue, 'was null');
			SELECT @valid = 0;
		END
		ELSE
		BEGIN
			...
			WHILE @i < LEN(@cValue)
			BEGIN
				SELECT @ch = SUBSTRING(@cValue, @i, 1);

				IF NOT (@ch BETWEEN '0' AND '9' OR @ch BETWEEN 'a' AND 'z')
				BEGIN
					--PRINT 'Rejected hair colour hex char:' + ISNULL(@ch, 'was null');
					SELECT @valid = 0;
				END
		...
		IF ISNULL(@cValue,'') NOT IN ('amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth')
		BEGIN
			--PRINT 'Rejected eye colour: ' + ISNULL(@cValue, 'was null');
			SELECT @valid = 0;
		END
		...
		IF LEN(@cValue) <> 9 OR TRY_CAST(@cValue AS int) IS NULL
		BEGIN
			--PRINT 'Rejected passport: ' + ISNULL(@cValue, 'was null');
			SELECT @valid = 0;
		END
	END
END
...
BEGIN
	...
	BEGIN
		...
		FROM	STRING_SPLIT(@line, ' ') [line]
		CROSS APPLY [AOC].[SplitAttribute]([line].[value]) [KVP];
	END
	ELSE
	BEGIN
		EXEC [dbo].[#ValidateAttributes] 0, @valid OUTPUT;

		IF @valid = 1
			SELECT @totalvalid += 1;

		EXEC [dbo].[#ValidateAttributes] 1, @valid OUTPUT;

		IF @valid = 1
			SELECT @totalfullyvalid += 1;
...
-- Part 1: 233
-- Part 2: 111
SELECT	[Part 1 valid] = @totalvalid
		,[Part 2 valid] = @totalfullyvalid;
